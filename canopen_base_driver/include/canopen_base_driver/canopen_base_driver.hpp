...
protected:
  ...
  {
    RCLCPP_INFO(
      this->get_logger(), "Received PDO index %hu subindex %hhu data %u", data.index_,
      data.subindex_, data.data_);
  }

  explicit BaseDriver(const rclcpp::NodeOptions & options) : DriverInterface("base_driver", options)
  {
  }
...
public:
  ...
  /**
     ...
     */
  void remove(
    ev::Executor & exec, canopen::AsyncMaster & master, uint8_t node_id) noexcept override;
};
}  // namespace ros2_canopen
