...
.. csv-table:: Development Objectives
  ...
  Easy Maintenance; Maintenance effort should be reduced as much as possible. Therefore, a clean and clear code structure and documentation is needed and only functionalities that are not already available from other high quality open source libraries should be self implemented.
  Enable controlling drives via ros2_control; A driver for CIA402 and a ros2_control interface need to be developed.
  Enable controlling drives via ros2 service interface; A driver for CIA402 and a service interface need to be developed.
