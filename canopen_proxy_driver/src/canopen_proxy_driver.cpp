...
namespace ros2_canopen
{
  ...
  ProxyDriver::on_configure(const rclcpp_lifecycle::State &state)
  {
    if (BaseDriver::on_configure(state) != rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn::SUCCESS)
    {
  ...
  rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn
  ProxyDriver::on_activate(const rclcpp_lifecycle::State &state)
  {
    return BaseDriver::on_activate(state);
  }

  rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn
  ProxyDriver::on_deactivate(const rclcpp_lifecycle::State &state)
  {
    return BaseDriver::on_deactivate(state);
  }

  rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn
  ProxyDriver::on_cleanup(const rclcpp_lifecycle::State &state)
  {
    return BaseDriver::on_cleanup(state);
  }

  rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn
  ProxyDriver::on_shutdown(const rclcpp_lifecycle::State &state)
  {
    return BaseDriver::on_shutdown(state);
  }
