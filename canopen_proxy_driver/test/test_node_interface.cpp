...
TEST(NodeCanopenProxyDriver, test_good_sequence_advanced)
{
  ...
  auto interface = new ros2_canopen::node_interfaces::NodeCanopenProxyDriver<rclcpp::Node>(node);
...
TEST(NodeCanopenBasicLifecycleMaster, test_good_sequence_advanced)
{
  ...
  auto interface =
    new ros2_canopen::node_interfaces::NodeCanopenProxyDriver<rclcpp_lifecycle::LifecycleNode>(
      node);
  auto exec = std::make_shared<rclcpp::executors::SingleThreadedExecutor>();
  exec->add_node(node->get_node_base_interface());
  std::thread spinner = std::thread([exec] { exec->spin(); });
