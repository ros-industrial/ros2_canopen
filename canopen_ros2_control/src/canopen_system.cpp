...
void CanopenSystem::initDeviceContainer()
{
  ...
  if (device_container_->init(
    ...
    for (auto it = reg_dr.begin(); it != reg_dr.end(); it++) {
      auto proxy_driver = std::static_pointer_cast<ros2_canopen::ProxyDriver>(
        device_container_->get_node(it->second.first));
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  for (auto it = canopen_data_.begin(); it != canopen_data_.end(); ++it) {
    auto proxy_driver =
      std::static_pointer_cast<ros2_canopen::ProxyDriver>(device_container_->get_node(it->first));
