  ...
  std::shared_ptr<ConfigurationManager> config)
{
  ...
  master_ = std::make_shared<LelyMasterBridge>(*exec_, *timer_, *chan_, dcf_txt, dcf_bin, nodeid);
  master_->Reset();

  spinner_ = std::thread([this]() { loop_->run(); });

  /// @todo add services
  sdo_read_service = this->create_service<canopen_interfaces::srv::COReadID>(
    std::string(this->get_name()).append("/sdo_read").c_str(),
    std::bind(
      &ros2_canopen::MasterNode::on_sdo_read, this, std::placeholders::_1, std::placeholders::_2));

  sdo_write_service = this->create_service<canopen_interfaces::srv::COWriteID>(
    std::string(this->get_name()).append("/sdo_write").c_str(),
    std::bind(
      &ros2_canopen::MasterNode::on_sdo_write, this, std::placeholders::_1, std::placeholders::_2));
}
void MasterNode::add_driver(
  std::shared_ptr<ros2_canopen::DriverInterface> node_instance, uint8_t node_id)
{
