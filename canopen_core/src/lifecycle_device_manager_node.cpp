...
void LifecycleDeviceManagerNode::init(std::shared_ptr<ros2_canopen::ConfigurationManager> config)
{
  this->config_ = config;
}

bool LifecycleDeviceManagerNode::load_from_config()
{
...
unsigned int LifecycleDeviceManagerNode::get_state(uint8_t node_id, std::chrono::seconds time_out)
{
...
bool LifecycleDeviceManagerNode::change_state(
...
bool LifecycleDeviceManagerNode::bring_up_master()
{
...
bool LifecycleDeviceManagerNode::bring_down_master()
{
...
bool LifecycleDeviceManagerNode::bring_up_driver(std::string device_name)
{
...
bool LifecycleDeviceManagerNode::bring_down_driver(std::string device_name)
{
  auto node_id = this->device_names_to_ids[device_name];
  auto master_state = this->get_state(master_id_);
...
bool LifecycleDeviceManagerNode::bring_up_all()
{
...
bool LifecycleDeviceManagerNode::bring_down_all()
{
  for (auto it = this->device_names_to_ids.begin(); it != this->device_names_to_ids.end(); ++it) {
    if (it->first.find("master") != std::string::npos) {
