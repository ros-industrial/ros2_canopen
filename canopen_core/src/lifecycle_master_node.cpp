...
LifecycleMasterNode::on_configure(const rclcpp_lifecycle::State &)
{
  ...
  sdo_read_service = this->create_service<canopen_interfaces::srv::COReadID>(
    ...
    std::bind(
      &ros2_canopen::LifecycleMasterNode::on_sdo_read, this, std::placeholders::_1,
      std::placeholders::_2));

  sdo_write_service = this->create_service<canopen_interfaces::srv::COWriteID>(
    std::string(this->get_name()).append("/sdo_write").c_str(),
    std::bind(
      &ros2_canopen::LifecycleMasterNode::on_sdo_write, this, std::placeholders::_1,
      std::placeholders::_2));
...
LifecycleMasterNode::on_activate(const rclcpp_lifecycle::State & state)
{
  ...
  master_ =
    std::make_shared<LelyMasterBridge>(*exec_, *timer_, *chan_, dcf_txt_, dcf_bin_, node_id_);
  master_->Reset();
...
void LifecycleMasterNode::init_driver(
  std::shared_ptr<ros2_canopen::LifecycleDriverInterface> node_instance, uint8_t node_id)
{
