...
void LifecycleDeviceContainerNode::add_node_to_executor(
  const std::string & driver_name, const uint16_t node_id, const std::string & node_name)
{
  if (auto exec = executor_.lock()) {
    exec->add_node(node_wrappers_[node_id].get_node_base_interface(), true);

    auto node_instance = std::static_pointer_cast<ros2_canopen::LifecycleDriverInterface>(
      node_wrappers_[node_id].get_node_instance());

    active_drivers_.insert({node_name, std::make_pair(node_id, driver_name)});

    RCLCPP_INFO(
      this->get_logger(), "Added node of type %s with name \"%s\" for node_id %hu to executor.",
      driver_name.c_str(), node_name.c_str(), node_id);
  } else {
    RCLCPP_ERROR(
      this->get_logger(), "Failed to add %s of type %s to executor", node_name.c_str(),
      driver_name.c_str());
  }
}

void LifecycleDeviceContainerNode::remove_node_from_executor(
  const std::string & driver_name, const uint16_t node_id, const std::string & node_name)
{
  ...
  if (auto exec = executor_.lock()) {
    ...
    RCLCPP_INFO(
      this->get_logger(), "Removed %s of type %s from executor", node_name.c_str(),
      driver_name.c_str());
  } else {
    RCLCPP_ERROR(
      this->get_logger(), "Failed to remove %s of type %s from executor", node_name.c_str(),
      driver_name.c_str());
  }
}
...
bool LifecycleDeviceContainerNode::load_component(
...
bool LifecycleDeviceContainerNode::init_master(uint16_t node_id)
{
  ...
  can_master_ = std::static_pointer_cast<ros2_canopen::LifecycleMasterInterface>(
    node_wrappers_[node_id].get_node_instance());
  can_master_->init();
...
bool LifecycleDeviceContainerNode::init_device_manager(uint16_t node_id)
{
  ...
  auto device_manager = std::static_pointer_cast<ros2_canopen::LifecycleDeviceManagerNode>(
    node_wrappers_[node_id].get_node_instance());
  device_manager->init(this->config_);
  device_manager->set_parameter(
    rclcpp::Parameter("container_name", this->get_fully_qualified_name()));
  return true;
}
...
bool LifecycleDeviceContainerNode::load_master_from_config()
{
  ...
  for (auto it = devices.begin(); it != devices.end(); it++) {
    if (it->find("master") != std::string::npos && !master_found) {
      ...
      auto res = this->registered_drivers_.emplace(
        *it, std::make_pair(node_id.value(), driver_name.value()));
      if (!res.second) {
        RCLCPP_ERROR(
          this->get_logger(), "Error: Bus Configuration has duplicate configuration for %s",
          it->c_str());
        return false;
      }
...
bool LifecycleDeviceContainerNode::load_drivers_from_config()
{
  ...
  for (auto it = devices.begin(); it != devices.end(); it++) {
    if (it->find("master") == std::string::npos) {
      ...
      add_node_to_executor(driver_name.value(), node_id.value(), *it);
      auto node_instance = std::static_pointer_cast<ros2_canopen::LifecycleDriverInterface>(
        node_wrappers_[node_id.value()].get_node_instance());
      node_instance->init();
...
int main(int argc, char const * argv[])
{
  ...
  std::thread spinThread([&exec]() { exec->spin(); });
