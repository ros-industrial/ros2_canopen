...
public:
  ...
  virtual void remove(
    ev::Executor & exec, canopen::AsyncMaster & master, uint8_t node_id) noexcept = 0;
};
...
public:
  ...
  /**
         ...
         */
  virtual void init_from_master(
    std::shared_ptr<ev::Executor> exec, std::shared_ptr<canopen::AsyncMaster> master,
  ...
  bool demand_init_from_master()
  {
    ...
    auto demand_init_from_master_client = this->create_client<canopen_interfaces::srv::CONode>(
    ...
    while (!demand_init_from_master_client->wait_for_service(non_transmit_timeout_)) {
      if (!rclcpp::ok()) {
        RCLCPP_ERROR(
          this->get_logger(), "Interrupted while waiting for init_driver service. Exiting.");
        return false;
      }
...
public:
  MasterInterface(const std::string & node_name, const rclcpp::NodeOptions & node_options)
  : rclcpp::Node(node_name, node_options)
  {
  }
  virtual void init(
    std::string dcf_txt, std::string dcf_bin, std::string can_interface_name, uint8_t nodeid,
    std::shared_ptr<ros2_canopen::ConfigurationManager> config)
  {
    dcf_txt_ = dcf_txt;
    dcf_bin_ = dcf_bin;
...
public:
  /**
         ...
         * @param nodeid
         */
  LifecycleMasterInterface(const std::string & node_name, const rclcpp::NodeOptions & node_options)
  : rclcpp_lifecycle::LifecycleNode(node_name, node_options)
  {
  }
  virtual void init() {}
  ...
  virtual void init_driver(
    std::shared_ptr<ros2_canopen::LifecycleDriverInterface>, uint8_t node_id) = 0;
};
