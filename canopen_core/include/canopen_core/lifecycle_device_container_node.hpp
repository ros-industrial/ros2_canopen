...
public:
  ...
  : rclcpp_components::ComponentManager(executor, node_name, node_options)
  {
    ...
    init_driver_service_ = this->create_service<canopen_interfaces::srv::CONode>(
      "~/init_driver", std::bind(
                         &LifecycleDeviceContainerNode::on_init_driver, this, std::placeholders::_1,
                         std::placeholders::_2));
  }
...
private:
  std::map<std::string, std::pair<uint16_t, std::string>>
    registered_drivers_;  ///< Map of drivers registered in busconfiguration. Name is key.
  std::map<std::string, std::pair<uint16_t, std::string>>
    active_drivers_;  ///< Map of drivers activated (added to ros executor). Name is key.
  std::map<uint16_t, rclcpp_components::NodeInstanceWrapper>
    node_wrappers_;  ///< Map of node instances. CAN id is key.
  std::shared_ptr<ros2_canopen::LifecycleMasterInterface>
