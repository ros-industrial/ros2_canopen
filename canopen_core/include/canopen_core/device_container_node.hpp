...
public:
    DeviceContainerNode(
        ...
        std::string node_name = "device_container_node",
        ...
        this->add_node_master_service_= this->create_service<canopen_interfaces::srv::CONode>(
            "~/add_driver_to_master", 
            std::bind(&DeviceContainerNode::on_add_driver_to_master, this, std::placeholders::_1, std::placeholders::_2)
            );
        this->remove_node_master_service_ = this->create_service<canopen_interfaces::srv::CONode>(
            "~/remove_driver_from_master",
            std::bind(&DeviceContainerNode::on_remove_driver_from_master, this, std::placeholders::_1, std::placeholders::_2)
            );
    }
...
private:
    ...
    rclcpp::Service<canopen_interfaces::srv::CONode>::SharedPtr add_node_master_service_;
     ...
     * 
     */
    bool add_driver_to_master(uint8_t node_id);
     ...
     * @param node_id       CANopen Id of the device the driver targets
     */
    bool remove_driver_from_master(uint8_t node_id);

    /**
     * @brief Service Callback for adding drivers to master
     * 
     * @param request 
     * @param response 
     */
    void on_add_driver_to_master(
            const std::shared_ptr<canopen_interfaces::srv::CONode::Request> request,
            std::shared_ptr<canopen_interfaces::srv::CONode::Response> response)
    {
        response->success = this->add_driver_to_master(request->nodeid);
    }

    /**
     * @brief Service Callback for removing drivers from master
     * 
     * @param request 
     * @param response 
     */
    void on_remove_driver_from_master(
            const std::shared_ptr<canopen_interfaces::srv::CONode::Request> request,
            std::shared_ptr<canopen_interfaces::srv::CONode::Response> response)
    {
        response->success = this->remove_driver_from_master(request->nodeid);
    }
     ...
     * 
     * @return std::map<uint8_t, std::string> 
     */
    std::map<uint8_t, std::string> list_components();
