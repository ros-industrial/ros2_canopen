...
/**
 * @brief Non Lifecycle Device Container for CANopen
 *
...
public:
  ...
  /**
     * @brief Executes the initialisation
     *
     * This will execute the initialization but read
...
private:
  ...
  std::map<std::string, std::pair<uint8_t, std::string>>
    active_drivers_;  ///< Map of drivers activated. Name is key.
  ...
  std::map<uint8_t, rclcpp_components::NodeInstanceWrapper>
    node_wrappers_;  ///< Map of node instances that are in the executor. CAN id is key.
  ...
  /**
     ...
     */
  bool load_component(
    std::string & package_name, std::string & driver_name, uint8_t node_id,
