...
void MotionControllerDriver::init(
  ...
  std::shared_ptr<ros2_canopen::ConfigurationManager> config) noexcept
{
  ...
  auto period =
    this->config_->get_entry<uint32_t>(std::string(this->get_name()), std::string("period"));
  if (!period.has_value()) {
    RCLCPP_ERROR(
      this->get_logger(), "ERROR: Bus Configuration does not set period for %s", this->get_name());
    return;
  }
  ...
  timer_ =
    this->create_wall_timer(2000ms, std::bind(&MotionControllerDriver::run, this), timer_group);
  driver->Boot();
  active.store(true);
}
